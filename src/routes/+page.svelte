<script>
	...
	const getCards = async () => {
		...
		cardsInfo.forEach((card) => {
			...
			cards.push({
				// For Kintone, we need to put .value after
		...
		<div class="left">
			...
			<div class="card-row">
				...
				<!-- TODO: Here, we'll need to loop through our cards Array, and display each card, with its title and color. -->
					<div class:blue-card={card.color === "Blue"} class:red-card={card.color === "Red" }>
						<p>{card.title}</p>
						<label>
							<input type="radio" value="Red" bind:group={card.color} name={i}>
							Red
						</label>
						<label>
							<input type="radio" value="Blue" bind:group={card.color} name={i}>
							Blue
						</label>
					</div>
				{/each}
